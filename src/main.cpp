...
uint256 hashGenesisBlock("0x96e9ee342a81adb2558e877cf0732ee413762da43018f555702f7d2ee69c99e9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MineCoin: starting difficulty is 1 / 2^12
...
{
    // MineCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // MineCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 60; // MineCoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // MineCoin: 30 seconds
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MineCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x96e9ee342a81adb2558e877cf0732ee413762da43018f555702f7d2ee69c99e9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1440696188;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1021531;
        ...
        assert(block.hashMerkleRoot == uint256("0x234383ad6f89ee376010cda0a0ffcdae20f772eb60d917eb1048a6686bcf789e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x3b, 0xa1, 0xc9, 0x2a }; // MineCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MineCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MineCoinMiner(CWallet *pwallet)
{
    printf("MineCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running MineCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MineCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MineCoinMiner, pwallet));
}
